...
func main() {
	...
	conf, err := jt.Read[setup.SendGridConfig]()
	if err != nil {
		log.Fatalf(mld.LogFmt, "Failed to read configuration.", err)
	}

	logger := slog.Default() // TODO Log level and structure configurable in config file for all executables.
	...
	server, err := setup.CreateSendGridProvider(ctx, conf, options)
	if err != nil {
